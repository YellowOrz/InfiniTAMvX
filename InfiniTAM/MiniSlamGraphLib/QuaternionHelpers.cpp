...
static int QuaternionFromRotationMatrix_variant(const double *matrix) {
  // NOTE: 来自文档《Representing Attitude: Euler Angles, Unit Quaternions, and Rotation Vectors》公式145
  int variant = 0;
...
void QuaternionHelpers::dQuaternion_dRotationMatrix(const double *matrix, double *dq_dR) {
  ...
  */
  // NOTE: 以下公式来自文档《Representing Attitude: Euler Angles, Unit Quaternions, and Rotation Vectors》
  int variant = QuaternionFromRotationMatrix_variant(matrix);

  const double &r11 = matrix[0], &r12 = matrix[1], &r13 = matrix[2];
  const double &r21 = matrix[3], &r22 = matrix[4], &r23 = matrix[5];
  const double &r31 = matrix[6], &r32 = matrix[7], &r33 = matrix[8];
  /*  varient 1: d = r11 + r22 + r33 + 1.0;
   *  [ 0.5/sqrt(d)
   */
  double denom = 0.0;
  if (variant == 0) {         // 公式(141)
    denom = r11 + r22 + r33 + 1.0f;
  } else if (variant == 1) {  // 公式(142)
    denom = r11 - r22 - r33 + 1.0f;
  } else if (variant == 2) {  // 公式(143)
    denom = -r11 + r22 - r33 + 1.0f;
  } else if (variant == 3) {  // 公式(144)
    denom = -r11 - r22 + r33 + 1.0f;
  }
  //??? 接下来是why？？？
  double sqrt_denom = sqrt(denom);
  double d_sqrt_denom_dR11, d_sqrt_denom_dR22, d_sqrt_denom_dR33;
  double part32_sign, part13_sign, part21_sign;
  ...
  if (variant == 0) {  // 公式(141)
    ...
    part32_sign = -1.0f, part13_sign = -1.0f, part21_sign = -1.0f;
    denom_index = 0;
    part32_index = 1;
    part13_index = 2;
    part21_index = 3;
