...
static bool pnm_readdata_binary(FILE *f, int xsize, int ysize, FormatType type, void *data) {
  ...
  switch (type) {
    case MONO_8u: bytesPerSample = sizeof(unsigned char);
      channels = 1;
      break;
    case RGB_8u: bytesPerSample = sizeof(unsigned char);
      channels = 3;
      break;
    case MONO_16s: bytesPerSample = sizeof(short);
      channels = 1;
      break;
    case MONO_16u: bytesPerSample = sizeof(unsigned short);
      channels = 1;
      break;
    case FORMAT_UNKNOWN:
    default: break;
...
static bool pnm_writeheader(FILE *f, int xsize, int ysize, FormatType type) {
  ...
  int max = 0;
  switch (type) {
    case MONO_8u: pnmid = pgm_id;
      max = 256;
      break;
    case RGB_8u: pnmid = ppm_id;
      max = 255;
      break;
    case MONO_16s: pnmid = pgm_id;
      max = 32767;
      break;
    case MONO_16u: pnmid = pgm_id;
      max = 65535;
      break;
    case FORMAT_UNKNOWN:
...
void SaveImageToFile(const ORUtils::Image<ORUtils::Vector4<unsigned char> > *image,
                     const char *fileName,
                     bool flipVertical) {
