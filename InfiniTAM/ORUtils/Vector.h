...
class Vector3 : public Vector3_<T> {
 public:
  ...
  _CPU_AND_GPU_CODE_ inline Vector3<unsigned char> toUChar() const {
    ...
    return Vector3<unsigned char>((unsigned char) CLAMP(vi.x, 0, 255),
                                  (unsigned char) CLAMP(vi.y, 0, 255),
...
class Vector4 : public Vector4_<T> {
 public:
  ...
  _CPU_AND_GPU_CODE_ inline Vector4<unsigned char> toUChar() const {
    ...
    return Vector4<unsigned char>((unsigned char) CLAMP(vi.x, 0, 255),
                                  (unsigned char) CLAMP(vi.y, 0, 255),
                                  (unsigned char) CLAMP(vi.z, 0, 255),
...
class Vector6 : public Vector6_<T> {
 public:
  ...
  _CPU_AND_GPU_CODE_ explicit Vector6(const Vector2_<T> &u, T v0, T v1, T v2, T v3) {
    ...
    this->v[2] = v0;
    this->v[3] = v1;
    this->v[4] = v2, this->v[5] = v3;
  }

  _CPU_AND_GPU_CODE_ inline Vector6<int> toIntRound() const {
    return Vector6<int>((int) ROUND(this[0]),
                        (int) ROUND(this[1]),
                        (int) ROUND(this[2]),
                        (int) ROUND(this[3]),
                        (int) ROUND(this[4]),
  ...
  _CPU_AND_GPU_CODE_ inline Vector6<unsigned char> toUChar() const {
    ...
    return Vector6<unsigned char>((unsigned char) CLAMP(vi[0], 0, 255),
                                  (unsigned char) CLAMP(vi[1], 0, 255),
                                  (unsigned char) CLAMP(vi[2], 0, 255),
                                  (unsigned char) CLAMP(vi[3], 0, 255),
                                  (unsigned char) CLAMP(vi[4], 0, 255),
                                  (unsigned char) CLAMP(vi[5], 0, 255));
  }

  _CPU_AND_GPU_CODE_ inline Vector6<float> toFloat() const {
    return Vector6<float>((float) this[0],
                          (float) this[1],
                          (float) this[2],
                          (float) this[3],
                          (float) this[4],
...
class VectorX : public VectorX_<T, s> {
 public:
  ...
  _CPU_AND_GPU_CODE_ explicit VectorX(const T *tp) {
    for (int i = 0; i < s; i++)
      this->v[i] = tp[i];
  } // Construct from array
  ...
  _CPU_AND_GPU_CODE_ VectorX<T, s> &setValues(const T *rhs) {
    for (int i = 0; i < s; i++) this->v[i] = rhs[i];
    return *this;
  }
