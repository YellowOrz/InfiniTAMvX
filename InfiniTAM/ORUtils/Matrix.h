...
class Matrix4 : public Matrix4_<T> {
 public:
  ...
  // get values
  _CPU_AND_GPU_CODE_ inline Vector4<T> getRow(int r) const {
    Vector4<T> v;
    for (int x = 0; x < 4; x++) v[x] = at(x, r);
    return v;
  }
  _CPU_AND_GPU_CODE_ inline Vector4<T> getColumn(int c) const {
    Vector4<T> v;
    memcpy(v.v, this->m + 4 * c, sizeof(T) * 4);
    return v;
  }
  ...
  _CPU_AND_GPU_CODE_ inline Matrix4 &operator+=(const T &r) {
    for (int i = 0; i < 16; ++i) this->m[i] += r;
    return *this;
  }
  _CPU_AND_GPU_CODE_ inline Matrix4 &operator-=(const T &r) {
    for (int i = 0; i < 16; ++i) this->m[i] -= r;
  ...
  _CPU_AND_GPU_CODE_ inline Matrix4 &operator+=(const Matrix4 &mat) {
    for (int i = 0; i < 16; ++i)
      this->m[i] += mat.m[i];
    return *this;
  }
...
class Matrix3 : public Matrix3_<T> {
 public:
  ...
  // get values
  _CPU_AND_GPU_CODE_ inline Vector3<T> getRow(int r) const {
    Vector3<T> v;
    for (int x = 0; x < 3; x++) v[x] = at(x, r);
    return v;
  }
  _CPU_AND_GPU_CODE_ inline Vector3<T> getColumn(int c) const {
    Vector3<T> v;
  ...
  _CPU_AND_GPU_CODE_ inline Matrix3 &operator+=(const T &r) {
    for (int i = 0; i < 9; ++i) this->m[i] += r;
    return *this;
  }
  _CPU_AND_GPU_CODE_ inline Matrix3 &operator-=(const T &r) {
    for (int i = 0; i < 9; ++i) this->m[i] -= r;
  ...
  _CPU_AND_GPU_CODE_ inline Matrix3 &operator+=(const Matrix3 &mat) {
    for (int i = 0; i < 9; ++i)
      this->m[i] += mat.m[i];
    return *this;
  }
  ...
  _CPU_AND_GPU_CODE_ inline T det() const {
    return (this->m11 * this->m22 - this->m12 * this->m21) * this->m00
        + (this->m12 * this->m20 - this->m10 * this->m22) * this->m01
        + (this->m10 * this->m21 - this->m11 * this->m20) * this->m02;
  }
...
class MatrixSQX : public MatrixSQX_<T, s> {
 public:
  _CPU_AND_GPU_CODE_ MatrixSQX() {
    this->dim = s;
    this->sq = s * s;
  }
  _CPU_AND_GPU_CODE_ MatrixSQX(T t) {
    this->dim = s;
    this->sq = s * s;
    setValues(t);
  }
  _CPU_AND_GPU_CODE_ MatrixSQX(const T *m) {
    this->dim = s;
    this->sq = s * s;
    setValues(m);
  }
  ...
  _CPU_AND_GPU_CODE_ inline void setIdentity() {
    setZeros();
    for (int i = 0; i < s * s; i++) this->m[i + i * s] = 1;
  }

  // get values
  _CPU_AND_GPU_CODE_ inline VectorX<T, s> getRow(int r) const {
    VectorX<T, s> v;
    for (int x = 0; x < s; x++) v[x] = at(x, r);
    return v;
  }
  _CPU_AND_GPU_CODE_ inline VectorX<T, s> getColumn(int c) const {
    Vector4<T> v;
    for (int x = 0; x < s; x++) v[x] = at(c, x);
    return v;
  }
  _CPU_AND_GPU_CODE_ inline MatrixSQX<T, s> getTranspose() { // transpose
  ...
  _CPU_AND_GPU_CODE_ inline MatrixSQX<T, s> &operator+=(const T &r) {
    for (int i = 0; i < s * s; ++i) this->m[i] += r;
    return *this;
  }
  _CPU_AND_GPU_CODE_ inline MatrixSQX<T, s> &operator-=(const T &r) {
    for (int i = 0; i < s * s; ++i) this->m[i] -= r;
  ...
  _CPU_AND_GPU_CODE_ inline MatrixSQX<T, s> &operator+=(const MatrixSQX<T, s> &mat) {
    for (int i = 0; i < s * s; ++i)
      this->m[i] += mat.m[i];
    return *this;
  }
