...
class MemoryBlock {
  ...
  */
  MemoryBlock(size_t dataSize, MemoryDeviceType memoryType) {
    ...
    switch (memoryType) {
      case MEMORYDEVICE_CPU: Allocate(dataSize, true, false, true);
        break;
  ...
  */
  ...
  void SetFrom(const MemoryBlock<T> *source, MemoryCopyDirection memoryCopyDirection) {
    ...
    switch (memoryCopyDirection) {
      case CPU_TO_CPU: memcpy(this->data_cpu, source->data_cpu, source->dataSize * sizeof(T));
        break;
#ifndef COMPILE_WITHOUT_CUDA
        case CPU_TO_CUDA:
...
#endif
      switch (allocType) {
        case 0: if (data_cpu != NULL) delete[] data_cpu;
          break;
        case 1:
#ifndef COMPILE_WITHOUT_CUDA
