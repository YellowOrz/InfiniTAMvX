...
void SE3Pose::SetParamsFromModelView() {
  ...
  this->params.each.rx = resultRot.x;
  this->params.each.ry = resultRot.y;
  this->params.each.rz = resultRot.z;
  this->params.each.tx = rottrans.x;
  this->params.each.ty = rottrans.y;
  this->params.each.tz = rottrans.z;
}
