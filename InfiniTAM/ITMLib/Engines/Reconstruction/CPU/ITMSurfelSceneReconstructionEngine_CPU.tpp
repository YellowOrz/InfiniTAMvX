...
template<typename TSurfel>
void ITMSurfelSceneReconstructionEngine_CPU<TSurfel>::AddNewSurfels(ITMSurfelScene<TSurfel> *scene,
                                                                    const ITMView *view,
                                                                    const ITMTrackingState *trackingState) const {
...
template<typename TSurfel>
void ITMSurfelSceneReconstructionEngine_CPU<TSurfel>::FindCorrespondingSurfels(const ITMSurfelScene<TSurfel> *scene,
                                                                               const ITMView *view,
...
#endif
  for (int locId = 0; locId < pixelCount; ++locId) {
    find_corresponding_surfel(locId,
                              invT,
                              depthMap,
                              depthMapWidth,
                              normalMap,
                              indexImageSuper,
                              sceneParams.supersamplingFactor,
                              surfels,
                              correspondenceMap,
                              newPointsMask);
  }
...
template<typename TSurfel>
void ITMSurfelSceneReconstructionEngine_CPU<TSurfel>::FuseMatchedPoints(ITMSurfelScene<TSurfel> *scene,
                                                                        const ITMView *view,
                                                                        const ITMTrackingState *trackingState) const {
  const Vector4u *colourMap = view->rgb->GetData(MEMORYDEVICE_CPU);
...
template<typename TSurfel>
void ITMSurfelSceneReconstructionEngine_CPU<TSurfel>::MergeSimilarSurfels(ITMSurfelScene<TSurfel> *scene,
                                                                          const ITMSurfelRenderState *renderState) const {
  const unsigned int *correspondenceMap = this->m_correspondenceMapMB->GetData(MEMORYDEVICE_CPU);
...
template<typename TSurfel>
void ITMSurfelSceneReconstructionEngine_CPU<TSurfel>::PreprocessDepthMap(const ITMView *view,
                                                                         const ITMSurfelSceneParams &sceneParams) const {
  const float *depthMap = view->depth->GetData(MEMORYDEVICE_CPU);
