...
_CPU_AND_GPU_CODE_ inline bool computePerPointGH_exDepth_Ab(THREADPTR(float) *A,
                                                            THREADPTR(float) &b,
  ...
  for (int para = 0; para < numPara; para++) {
    ...
    switch (para + (shortIteration && !rotationOnly ? 3 : 0)) {
      ...
      case 5: //tz
        ...
        break;
      default: d_point_col = Vector3f(0, 0, 0); // Should never happen
        break;
    };
...
template<bool shortIteration, bool rotationOnly, bool useWeights>
_CPU_AND_GPU_CODE_ inline bool computePerPointGH_exDepth(THREADPTR(float) *localNabla,
                                                         THREADPTR(float) *localHessian,
                                                         ...
                                                         const CONSTPTR(float) &depth,
                                                         THREADPTR(float) &depthWeight,
                                                         CONSTPTR(Vector2i) &viewImageSize,
                                                         const CONSTPTR(Vector4f) &viewIntrinsics,
                                                         const CONSTPTR(Vector2i) &sceneImageSize,
                                                         const CONSTPTR(Vector4f) &sceneIntrinsics,
                                                         const CONSTPTR(Matrix4f) &approxInvPose,
  ...
  bool ret = computePerPointGH_exDepth_Ab<shortIteration, rotationOnly, useWeights>(A,
                                                                                    b,
                                                                                    x,
                                                                                    y,
                                                                                    depth,
...
_CPU_AND_GPU_CODE_ inline bool computePerPointProjectedColour_exRGB(
  ...
  if (pt_image_proj.x < 0 || pt_image_proj.x >= imageSize.x - 1 ||
      pt_image_proj.y < 0 || pt_image_proj.y >= imageSize.y - 1)
    return false;
