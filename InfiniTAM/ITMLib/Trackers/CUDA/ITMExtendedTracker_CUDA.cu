...
__global__ void exRGBTrackerProjectPrevImage_device(Vector4f *out_points,
                                                    float *out_rgb,
                                                    const float *in_rgb,
...
int ITMExtendedTracker_CUDA::ComputeGandH_RGB(float &f, float *nabla, float *hessian, Matrix4f approxInvPose) {
  ...
  switch (currentIterationType) {
    case TRACKER_ITERATION_ROTATION: exRGBTrackerOneLevel_g_rt_device<true, true> << < gridSize, blockSize >> >(args);
      ORcudaKernelCheck;
      break;
    case TRACKER_ITERATION_TRANSLATION:
      ...
      ORcudaKernelCheck;
      break;
    case TRACKER_ITERATION_BOTH: exRGBTrackerOneLevel_g_rt_device<false, false> << < gridSize, blockSize >> >(args);
      ORcudaKernelCheck;
      break;
    default: break;
  }
...
void ITMExtendedTracker_CUDA::ProjectCurrentIntensityFrame(ITMFloat4Image *points_out,
  ...
  exRGBTrackerProjectPrevImage_device<<<gridSize, blockSize>>>(pointsOut,
                                                               intensityOut,
                                                               intensityIn,
                                                               depths,
                                                               imageSize_rgb,
...
// device functions
template<bool shortIteration, bool rotationOnly, bool useWeights>
__device__ void exDepthTrackerOneLevel_g_rt_device_main(ITMExtendedTracker_CUDA::AccuCell *accu,
                                                        float *depth,
                                                        Matrix4f approxInvPose,
                                                        Vector4f *pointsMap,
                                                        Vector4f *normalsMap,
  ...
  if (x < viewImageSize.x && y < viewImageSize.y) {
    isValidPoint = computePerPointGH_exDepth_Ab<shortIteration, rotationOnly, useWeights>(A,
                                                                                          b,
                                                                                          x,
                                                                                          y,
                                                                                          depth[x
...
template<bool shortIteration, bool rotationOnly, bool useWeights>
__global__ void exDepthTrackerOneLevel_g_rt_device(ITMExtendedTracker_KernelParameters_Depth para) {
  exDepthTrackerOneLevel_g_rt_device_main<shortIteration, rotationOnly, useWeights>(para.accu,
                                                                                    para.depth,
                                                                                    para.approxInvPose,
                                                                                    para.pointsMap,
                                                                                    para.normalsMap,
