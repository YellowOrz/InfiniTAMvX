...
void ITMLib::writeDisparityCalib(std::ostream &dest, const ITMDisparityCalib &src) {
  switch (src.GetType()) {
    case ITMDisparityCalib::TRAFO_AFFINE: dest << "affine " << src.GetParams().x << ' ' << src.GetParams().y << '\n';
      break;
    case ITMDisparityCalib::TRAFO_KINECT: dest << src.GetParams().x << ' ' << src.GetParams().y << '\n';
      break;
    default: throw std::runtime_error("Error: Unknown disparity calibration type");
  }
}
