...
ITMBasicSurfelEngine<TSurfel>::ITMBasicSurfelEngine(const ITMLibSettings *settings,
  ...
  surfelVisualisationEngine =
      ITMSurfelVisualisationEngineFactory<TSurfel>::make_surfel_visualisation_engine(deviceType);
...
template<typename TSurfel>
ITMTrackingState::TrackingResult ITMBasicSurfelEngine<TSurfel>::ProcessFrame(ITMUChar4Image *rgbImage,
  ...
  if (settings->behaviourOnFailure == ITMLibSettings::FAILUREMODE_RELOCALISE) {
    ...
    if (!hasAddedKeyframe && trackerResult == ITMTrackingState::TRACKING_FAILED) {
      ...
      surfelVisualisationEngine->FindSurfaceSuper(surfelScene,
                                                  trackingState->pose_d,
...
ITMBasicSurfelEngine<TSurfel>::ToSurfelImageType(GetImageType getImageType) {
  switch (getImageType) {
    ...
    case InfiniTAM_IMAGE_FREECAMERA_COLOUR_FROM_VOLUME: return ITMSurfelVisualisationEngine<TSurfel>::RENDER_COLOUR;
    case InfiniTAM_IMAGE_COLOUR_FROM_NORMAL:
    case InfiniTAM_IMAGE_FREECAMERA_COLOUR_FROM_NORMAL: return ITMSurfelVisualisationEngine<TSurfel>::RENDER_NORMAL;
    case InfiniTAM_IMAGE_COLOUR_FROM_CONFIDENCE:
    case InfiniTAM_IMAGE_FREECAMERA_COLOUR_FROM_CONFIDENCE: return ITMSurfelVisualisationEngine<TSurfel>::RENDER_CONFIDENCE;
    default: return ITMSurfelVisualisationEngine<TSurfel>::RENDER_LAMBERTIAN;
  }
}

template<typename TSurfel>
void ITMBasicSurfelEngine<TSurfel>::GetImage(ITMUChar4Image *out,
  ...
  switch (getImageType) {
    case ITMBasicSurfelEngine::InfiniTAM_IMAGE_ORIGINAL_RGB: out->ChangeDims(view->rgb->noDims);
      if (settings->deviceType == ITMLibSettings::DEVICE_CUDA)
        out->SetFrom(view->rgb, ORUtils::MemoryBlock<Vector4u>::CUDA_TO_CPU);
      else out->SetFrom(view->rgb, ORUtils::MemoryBlock<Vector4u>::CPU_TO_CPU);
      break;
    case ITMBasicSurfelEngine::InfiniTAM_IMAGE_ORIGINAL_DEPTH: out->ChangeDims(view->depth->noDims);
    ...
    case ITMBasicSurfelEngine::InfiniTAM_IMAGE_COLOUR_FROM_CONFIDENCE: {
      ...
      surfelVisualisationEngine->FindSurface(surfelScene,
                                             trackingState->pose_d,
                                             &view->calib.intrinsics_d,
                                             useRadii,
                                             USR_DONOTRENDER,
                                             surfelRenderState_live);
      surfelVisualisationEngine->RenderImage(surfelScene,
                                             trackingState->pose_d,
                                             surfelRenderState_live,
                                             out,
