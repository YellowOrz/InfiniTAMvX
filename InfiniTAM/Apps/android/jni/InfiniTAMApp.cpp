...
//		glViewport(0, 0, newWidth, newHeight);
  ...
  if (mRecordingMode)
    winImageType[0] = recordRGB ? ITMMainEngine::InfiniTAM_IMAGE_ORIGINAL_RGB
    ...
    glPushMatrix();
    {
      ...
      for (int w = 0; w < localNumWin; w++) {
        ...
        glTexImage2D(GL_TEXTURE_2D,
                     ...
                     GL_RGBA,
                     GL_UNSIGNED_BYTE,
                     outImage[w]->GetData(MEMORYDEVICE_CPU));
...
/*	const char *calibFile = "/storage/sdcard0/InfiniTAM/CAsmall/calib.txt";
  ...
  if (useLiveCamera == 0) {
    ...
    mImageSource =
        new InfiniTAM::Engine::ImageFileReader<InfiniTAM::Engine::ImageMaskPathGenerator>(calibFile, pathGenerator);
  ...
  mMainEngine = new ITMBasicEngine<ITMVoxel, ITMVoxelIndex>(mInternalSettings,
                                                            &mImageSource->calib,
                                                            mImageSource->getRGBImageSize(),
                                                            mImageSource->getDepthImageSize());
...
bool InfiniTAMApp::ProcessFrame(void) {
  ...
  if (mRecordingMode) {
    if (recordRGB) {
      if (colorVideoWriter == NULL) {
        ...
        colorVideoWriter->open("/sdcard/InfiniTAM/out_color.avi",
    ...
    if (depthVideoWriter == NULL) {
      ...
      depthVideoWriter->open("/sdcard/InfiniTAM/out_depth.avi",
  ...
  __android_log_print(ANDROID_LOG_VERBOSE,
                      "InfiniTAM",
                      "Process Frame finished: %f %f",
                      sdkGetTimerValue(&timer_instant),
                      sdkGetAverageTimerValue(&timer_average));
