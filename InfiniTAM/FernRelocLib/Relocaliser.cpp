...
Relocaliser::Relocaliser(ORUtils::Vector2<int> imgSize,
...
int Relocaliser::ProcessFrame(const ORUtils::Image<float> *img,
...
void Relocaliser::SaveToDirectory(const std::string &outputDirectory) {
  ...
  ofs << "type=rgb,levels=4,numFerns=" << encoding->getNumFerns() << ",numDecisionsPerFern="
      << encoding->getNumDecisions() / 3 << ",harvestingThreshold=" << keyframeHarvestingThreshold;
