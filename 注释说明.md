...
- 子图（submap）：在开启全局优化的情况下，重建好的三维模型可能需要“变形调整”。InfiniTAM跟BundleFusion采用的策略一样，将输入的每一帧融合成一个个独立、刚体的三维模型（即子图，submap）而不是都直接融合进大的三维模型（即最后的三维模型）。全局优化将优化这些子图的位姿。等重建结束后，将这些子图融合成一个大的三维模型即可。
    - InfiniTAM的子图构建策略是相机一旦离开当前子图一定范围（具体是xxxxx），就构建新的子图；而BundleFusion是连续11帧构建一个子图（如果我没有记错的话）
    - InfiniTAM中，正在使用（用于跟踪）的子图称为activate submap，暂时没用到（比如很远的）的子图称为passive map
- ？？？Primary Data（或者叫primary submap）: 在使用子图（submap）的重建模式中，当前输入帧融合进入的子图就是primary submap。primary submap也是所有activate submap中的一个
- 子图（submap）id：
    - 活跃子图id：存储活跃子图的数组 中的id
    - 全局子图id：存储所有子图（包含活跃和不活跃）的数组的id
- 跟踪得到的单帧位姿都是
- 子图之间的关联：
    - constraint: 每跟踪成功一帧，涉及的两个子图就会以该帧的位姿作为一个constraint。一次可以有多个constraint。是单向的。只有其他子图到主子图的constraint，没有反过来的？（可以说constraint就是个临时变量？）
    - link（也叫observation）: 通过huber函数从上面的多个constraint中加权求和到一个位姿，作为这两个子图的link。一次只有一个link。是单向的。主子图会记录与其他子图的link（其中的位姿是其他子图=>主子图），反之亦然。在有冲突的时候（比如回环检测），选择相信主子图的。只有在回环检测和新建子图的时候，会更新link？
- 子图中的位姿
    - 世界坐标系：整个扫描过程的第1帧在原点的坐标系
    - 相机位姿：默认的“相机位姿”都是指世界坐标系到当前帧的位姿，从即$T_{lw}$
    - 子图坐标系：整个子图的第1帧在原点的坐标系
    - 子图位姿：世界坐标系下到子图的第1帧（对应子图坐标系） 的位姿，即$T_{sw}$
    - 子图中的相机位姿：子图坐标系到当前帧的位姿，即$T_{ls}$
# 代码跟论文的区别

| 论文                                                         | 代码                                                         |
| ------------------------------------------------------------ | ------------------------------------------------------------ |
| Fig4中说的顺序是integration => swap in => raycast => swap out | 代码中的顺序是ntegration => swap in => swap out => raycast   |
| 相机出了当前子图的中间范围后新建子图                         | 主子图中voxel block（只看前1000个）的可见比例<阈值（0.4）就新建子图 |
|                                                              |                                                              |
...
├── MiniSlamGraphLib：位姿图优化
└── ORUtils：各种杂七杂八的基础模块的实现，比如矩阵、图片、SE3、Cholesky分解、内存块、文件读写、NVIDA计时器等

# Release

- version 0.2：完成InfiniTAM v3中子图、全局优化、位姿图优化的注释
