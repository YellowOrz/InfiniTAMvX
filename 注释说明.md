...
- 子图之间的关联：
    - constrain: 每跟踪成功一帧，涉及的两个子图就会以该帧的位姿作为一个constrain。一次可以有多个constrain。是单向的。只有其他子图到主子图的constrain，没有反过来的？（可以说constrain就是个临时变量？）
    - link（也叫observation）: 通过huber函数从上面的多个constrain中加权求和到一个位姿，作为这两个子图的link。一次只有一个link。是单向的。主子图会记录与其他子图的link（其中的位姿是其他子图=>主子图），反之亦然。在有冲突的时候（比如回环检测），选择相信主子图的。只有在回环检测和新建子图的时候，会更新link？
    - 
# 代码跟论文的区别
