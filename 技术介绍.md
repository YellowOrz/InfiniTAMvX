...
- 对应的数据结构是`ITMVoxel_[s/f]_[/rgb/conf]`
...
- 步骤
  ...
  - 在activate submap之间添加约束；将passive submap与primary submap添加约束，用来loop closure detection（利用上面的relocalization）
...
- 命名空间`ITMLib`：pipeline中的几个大模块主要都是在中，此外还包含
  ...
  - `ITMLowLevelEngine`：图像基础操作（例如图片的copy、gradient、rescale）
  - `ITMViewBuilder`：输入图片的预处理（例如depth图的类型转换、法向量计算、双边滤波等）
